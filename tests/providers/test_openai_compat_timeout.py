...
def _assert_openai_compat_local_timeout(timeout) -> None:
    assert timeout == 90.0
...
def test_openai_compat_provider_sets_timeout_on_local_http_client() -> None:
    ...
    _assert_openai_compat_local_timeout(client_kwargs["timeout"])
    assert client_kwargs["limits"].keepalive_expiry == 0

    openai_kwargs = mock_async_openai.call_args.kwargs
    _assert_openai_compat_local_timeout(openai_kwargs["timeout"])
...
def test_openai_compat_provider_uses_local_timeout_override(monkeypatch) -> None:
    spec = ProviderSpec(
        name="local",
        keywords=(),
        env_key="",
    ...
    monkeypatch.setenv("NANOBOT_OPENAI_COMPAT_TIMEOUT_S", "45")
    monkeypatch.setenv("NANOBOT_OPENAI_COMPAT_LOCAL_TIMEOUT_S", "20")
