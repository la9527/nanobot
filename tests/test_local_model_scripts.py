...
def test_local_models_help_lists_supported_targets():
    ...
    assert "lfm25-8b-a1b" in output.lower()
...
def test_lfm25_wrapper_uses_rapid_mlx_and_official_repo():
    repo_root = Path(__file__).resolve().parents[2]
    script_path = repo_root / "infra/scripts/local-models/start-lfm25-8b-a1b-rapid-mlx.sh"

    content = script_path.read_text(encoding="utf-8")

    assert 'LiquidAI/LFM2.5-8B-A1B' in content
    assert 'rapid-mlx' in content
    assert 'vllm_mlx.cli serve "$MODEL_REPO"' in content
    assert '--no-thinking' in content


def test_lfm25_vl_wrapper_uses_bootstrap_server_and_local_model_path():
    repo_root = Path(__file__).resolve().parents[2]
    script_path = repo_root / "infra/scripts/local-models/start-lfm25-vl-1.6b-mlx.sh"

    content = script_path.read_text(encoding="utf-8")

    assert 'liquidai-lfm25-vl-1_6b' in content
    assert 'mlx-vlm-server-bootstrap.py' in content
    assert '--vision-cache-size "$VISION_CACHE_SIZE"' in content


def test_mlx_vlm_bootstrap_logs_runtime_memory_samples():
    repo_root = Path(__file__).resolve().parents[2]
    script_path = repo_root / "infra/scripts/local-models/mlx-vlm-server-bootstrap.py"

    content = script_path.read_text(encoding="utf-8")

    assert "[Runtime memory]" in content
    assert '"ps", "-o", "rss="' in content
    assert "peak_rss=" in content
...
def test_start_local_model_services_refreshes_local_wrapper_scripts():
    ...
    assert 'start_manual_model_process "$model" "$attempts"' in content


def test_install_local_model_services_can_fallback_to_manual_process():
    repo_root = Path(__file__).resolve().parents[2]
    script_path = repo_root / "infra/scripts/local-models/install-launchd-services.sh"

    content = script_path.read_text(encoding="utf-8")

    assert 'start_manual_model_process "$model" "$attempts"' in content
    assert '(manual fallback)' in content
