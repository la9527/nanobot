...
def test_gateway_heartbeat_executes_on_internal_channel_even_with_websocket_target(
    monkeypatch, tmp_path: Path
) -> None:
    config_file = _write_instance_config(tmp_path)
    config = Config()
    ...
    class _FakeSessionManager:
        ...
        def list_sessions(self) -> list[dict[str, object]]:
            return [{"key": "websocket:alpha", "metadata": {}}]
    ...
    response = asyncio.run(on_execute("Morning briefing: deliver the scheduled briefing now."))

    assert response == "아침 브리핑 전달"
    assert captured["heartbeat_session_key"] == "heartbeat"
    assert captured["saved_heartbeat_session"] is not None
    process_kwargs = captured["process_direct_kwargs"]
    assert process_kwargs == {
