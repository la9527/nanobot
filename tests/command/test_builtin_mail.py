...
from nanobot.automation.mail import MAIL_SEND_APPROVAL_METADATA_KEY, N8NGmailAutomationConfig
...
class _Runner:
    def __init__(self):
        ...
        self.client = self
        self.config = N8NGmailAutomationConfig(base_url="http://127.0.0.1:5678", locale="ko-KR")
...
@pytest.mark.asyncio
async def test_cmd_mail_without_args_shows_help_with_status_flow(tmp_path: Path) -> None:
    ctx = _make_ctx(tmp_path, "/mail")

    out = await cmd_mail(ctx)

    assert "## Mail Automation" in out.content
    assert "`/mail status` to review Gmail webhook configuration and pending send approval" in out.content
    assert "Recommended flow:" in out.content


@pytest.mark.asyncio
async def test_cmd_mail_status_includes_config_and_pending_state(tmp_path: Path) -> None:
    ctx = _make_ctx(tmp_path, "/mail status")
    ctx.session.metadata[MAIL_SEND_APPROVAL_METADATA_KEY] = {
        "subject": "Budget Follow-up",
    ...
    assert "- Automation: configured" in out.content
    assert "- Base URL: http://127.0.0.1:5678" in out.content
    assert "- Pending: send approval" in out.content
    assert "- Subject: Budget Follow-up" in out.content
    assert "- Resolve with: /mail approve or /mail deny" in out.content


def test_build_help_text_mentions_mail_command() -> None:
    assert "/mail [subcommand] — Review Gmail setup, inspect mail, draft replies, and approve sends." in build_help_text()