...
def test_run_photos_mcp_app_dry_run_reports_shared_bundle_and_legacy_servers() -> None:
    ...
    # photo-ranker's runtime/vlm-cache roots can be redirected via env override
    # (e.g. to an external volume for disk space) independent of the shared
    # ~/.photos-mcp tree, so only assert they resolve to a valid absolute path.
    assert Path(payload["photo_ranker_runtime_root"]).is_absolute()
    assert Path(payload["photo_ranker_vlm_cache_root"]).is_absolute()
