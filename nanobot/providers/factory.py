    ...
    *,
    ...
    if backend == "openai_codex":
        ...
        provider = OpenAICompatProvider(
            ...
            prepare_command=p.prepare_command if p else None,
            prepare_timeout_s=p.prepare_timeout_s if p else 300.0,
            activity_command=p.activity_command if p else None,
            activity_timeout_s=p.activity_timeout_s if p else 30.0,
            tool_choice_mode=p.tool_choice_mode if p else "standard",
        )
    ...
    *,
    ...
    def _fallback_signature(fallback: ModelPresetConfig) -> tuple[object, ...]:
        ...
        return (
            ...
            fp.prepare_command if fp else None,
            fp.prepare_timeout_s if fp else 300.0,
            fp.activity_command if fp else None,
            fp.activity_timeout_s if fp else 30.0,
            fp.tool_choice_mode if fp else "standard",
    ...
    return (
        ...
        p.prepare_command if p else None,
        p.prepare_timeout_s if p else 300.0,
        p.activity_command if p else None,
        p.activity_timeout_s if p else 30.0,
        p.tool_choice_mode if p else "standard",
