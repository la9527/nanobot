...
smart-router 는 별도 special case 가 아니라 runtime plugin 이 기여하는 named target 으로 노출된다.

즉 아래 설정이 유효하면 `/model` 목록에 `smart-router` 가 자동으로 나타난다.
...
이때 `/model smart-router` 를 선택하면 실제 turn 실행은 smart-router plugin 이 해석한 tier 로 진행된다.
...
- smart-router 실험이 필요할 때만 `/model smart-router` 로 세션 override 를 건다.
